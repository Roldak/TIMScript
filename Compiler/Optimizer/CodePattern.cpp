    ...
    namespace opt{
        ...
        std::vector<CodePattern*> createConstantsOptimizationsPatterns(){
            ...
            patterns.push_back(new CodePattern({
                ...
                P_LIT(PUSH_I), P_FUN([](std::map<std::string, TSINSTR>& vars){
                    switch (vars["x"]) {
                        case ADD_I: return vars["a"]+vars["b"];
                        case SUB_I: return vars["a"]-vars["b"];
                        case MUL_I: return vars["a"]*vars["b"];
                        case DIV_I: return vars["a"]/vars["b"];
                        case MOD_I: return vars["a"]%vars["b"];
                        case POW_I: return (TSINSTR)pow(vars["a"], vars["b"]);
                            
                        default:    return vars["a"];
                    }
                })
            }));
            return patterns;
