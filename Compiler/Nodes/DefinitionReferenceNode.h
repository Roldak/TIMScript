...
namespace ts {
	...
	namespace nodes {
		...
		public:
			...
			inline cmplr::Definition* definition() {
				return _def;
			}
			inline void setDefinition(cmplr::Definition* def) {
				_def = def;
			}

			virtual void nextOverloadedDefinition() {
				_def = _def->getNextOverloadedDefinition();
			}

			virtual size_t definitionIndex() {
				return _def->index();
			}
		...
		public:
			...
			inline objects::NativeData* getData() {
				return _data._data;
			}
