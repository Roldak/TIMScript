...
namespace ts {
	namespace nodes {
		...
		public:
			...
			inline AbstractNode* node() {
				return _node;
			}
			inline void setNode(AbstractNode* n) {
				_node = n;
			}
		...
		public:
			...
			inline type::FunctionType* getSignature() {
				return _signature;
			}

			inline const std::vector<Argument*>& arguments() {
				return _args;
			}

			inline size_t getDebugInfoNodeIndex() {
				return _dbgInfoIndex;
			}
