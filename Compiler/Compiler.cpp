    ...
    namespace cmplr{
        ...
        AbstractNode* Compiler::parseStatement(){
            ...
            if(IS(TK_KEYWORD)){
                ...
                accept();
                switch (t) {
                    case BEGIN:     return new BeginNode(CUR_POSLENGTH, parseBlock());
                    case LET:       return parseVariableDeclaration(init_pos, init_len);
                    case DEF:       return parseDefinition();
                    case TYPEDEF:   return parseTypedef();
                    case PRINT:     return parsePrintNode(init_pos, init_len);
                    case IF:        return parseIfNode(init_pos, init_len);
                    case WHILE:     return parseWhileNode(init_pos, init_len);
                    case FOR:       return parseForNode(init_pos, init_len);
                    case FOREACH:   return parseForeachNode(init_pos, init_len);
                    case CLASS:     return parseClassDeclaration(false);
                    case INTERFACE: return parseInterfaceDeclaration();
                    case ABSTRACT:  if(accept(CLASS)) return parseClassDeclaration(true);
                    case RETURN:    return new ExplicitReturnNode(CUR_POSLENGTH, parseExpression());
                    case SCOPE:     return new ScopeNode(CUR_POSLENGTH, parseBlock());
                    case ASYNC:     return parseAsyncNode();
                    case MATCH:     return parseMatchNode();
                    case INCLUDE:   return parseInclude();
                    case IMPORT:    return parseImport();
                    case TRY:       return parseTryCatchNode();
                    case KTHROW:    return new ThrowNode(CUR_POSLENGTH, parseExpression());
                    case STRUCT:    return parseStructureDeclaration();
                    default:
                        break;
                }
            }
        ...
        AbstractNode* Compiler::parseSpecialBinaryExp(AbstractNode* left, tok::OPERATOR_TYPE op){
            using namespace tok;
            switch (op) {
                case COLON:     return new HardCastNode(left->pos(), left->len(), left, parseTypeDeclaration());
                case PAREN_O:   return new CallNode(left->pos(), left->len(), left, parseCallArguments());
                case DOT:       return new DotOperationNode(left->pos(), left->len(), left, expectIdentifier("Expected Identifier"));
                case BRACKET_O:{
        ...
        AbstractNode* Compiler::makeBinaryExp(AbstractNode* l, tok::OPERATOR_TYPE op, AbstractNode* r){
            ...
            switch (op) {
                case PLUS:          return new BinaryAdd(pos, len, l, r);
                case MINUS:         return new BinarySub(pos, len, l, r);
                case TIMES:         return new BinaryMul(pos, len, l, r);
                case SLASH:         return new BinaryDiv(pos, len, l, r);
                case MODULO:        return new BinaryMod(pos, len, l, r);
                case POWER:         return new BinaryPow(pos, len, l, r);
                case INF:           return new BinaryLessThan(pos, len, l, r);
                case SUP:           return new BinaryGreaterThan(pos, len, l, r);
                case EQUAL_EQUAL:   return new BinaryEqualEqual(pos, len, l, r);
                case NOT_EQUAL:     return new BinaryNotEqual(pos, len, l, r);
                case AND:           return new BinaryAnd(pos, len, l, r);
                case OR:            return new BinaryOr(pos, len, l, r);
                case ISTYPE:        return new BinaryIs(pos, len, l, r);
                case EQUAL:         return new AssignementNode(pos, len, (AssignableNode*)l, r);
        ...
        AbstractNode* Compiler::parseUnaryExp(){
            ...
            accept();
            switch (op) {
                case MINUS:         return new UnaryMinus(CUR_POSLENGTH, PARSE_UNR_PRIMARY);
                case PLUS_PLUS:     return new UnaryPlusPlus(CUR_POSLENGTH, (AssignableNode*)PARSE_UNR_PRIMARY);
                case MINUS_MINUS:   return new UnaryMinusMinus(CUR_POSLENGTH, (AssignableNode*)PARSE_UNR_PRIMARY);
                default:
        ...
        AbstractNode* Compiler::parsePrimary(){
            ...
            switch ((*_currentToken)->getType()) {
                case TK_BOOLEAN:    n=new BooleanNode(CUR_POSLENGTH, CAST(BooleanConstToken)->value()); accept(); return n;
                case TK_INTEGER:    n=new IntegerNode(CUR_POSLENGTH, CAST(IntegerConstToken)->value()); accept(); return n;
                case TK_REAL:       n=new RealNode(CUR_POSLENGTH, CAST(RealConstToken)->value()); accept(); return n;
                case TK_STRING:     n=parseStringConstant(); accept(); return n;
                case TK_IDENTIFIER: return parseIdentifier();
                case TK_KEYWORD:    return parsePrimaryKeyword();
                case TK_OPERATOR:   return parsePrimaryOperator();

                default: ERROR("Unexpected token : "+(*_currentToken)->toString()); return n;
            }
        }
